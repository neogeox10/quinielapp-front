    ...
    useEffect(() => {
        ...
        quiniela.partidos.forEach((p: any) => {
            if (p.fase === quiniela.faseActual) {
                const prediccion = quiniela.predicciones?.find((pp: any) => pp.partidoId === p._id);
                estadoInicial[p._id] = {
                    local: typeof prediccion?.golesLocal !== 'undefined' ? prediccion?.golesLocal : '',
                    visitante: typeof prediccion?.golesVisitante !== 'undefined' ? prediccion?.golesVisitante : '',
                    fase: p.fase
                };
            }
        });
